...
BEGIN
	...
	BEGIN -- Special Setting
		...
		BEGIN
			...
			INSERT INTO @list
			SELECT @masterId, @masterType, [dbo].FNAGetCommission(@masterId, @masterType, @collAmt, @payAmt, @serviceCharge, @tAmt, @sAgentComm, 'p', NULL, NULL, NULL, NULL), commissionCurrency FROM scPayMaster WHERE scPayMasterId = @masterId
			RETURN
		END
