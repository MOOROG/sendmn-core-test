...
AS
	...
	SET @SQL = ' SELECT 
		 [SN] = ROW_NUMBER() over (order by rt.ControlNo)
		,[Tran No] = ''<a class = "link" onclick ="ViewTranDetail('' + CAST(ISNULL(rt.holdTranId,RT.ID) AS VARCHAR(50)) + '');">'' + CAST(ISNULL(rt.holdTranId,RT.ID) AS VARCHAR(50)) + ''</a>''
