...
BEGIN  
 ...
 BEGIN  
  ...
  INSERT @columnList(columnName)  
   ...
   SELECT 'EX RATE CALCULATE BY PARTNER'   
   ...
   SET @table='(  
        ...
        ,[IS REAL TIME]      = CASE WHEN MAIN.isRealTime = 0 THEN ''FALSE'' ELSE ''TRUE'' END  
		,[IS ACTIVE]      = CASE WHEN MAIN.isActive = 0 THEN ''FALSE'' ELSE ''TRUE'' END  
        ,[MINIMUM LIMIT]     = MAIN.minTxnLimit  
        ,[MAXIMUM LIMIT]     = MAIN.maxTxnLimit  
        ,[LIMIT CURRENCY]     = MAIN.LimitCurrency    
        ,[EX RATE CALCULATE BY PARTNER]  = CASE WHEN MAIN.exRateCalByPartner = 0 THEN ''FALSE'' ELSE ''TRUE'' END  
        FROM ' + @tableName + ' MAIN WITH(NOLOCK)  
