...
{
    public class MessageSettingDao : RemittanceDao
    {
        ...
        public DataRow GetNewsFeederMessage(string msgId)
        {
            var sql = "EXEC [proc_messageDisplay] @flag = 'getNewsfeederById'";
            sql += ", @msgId =" + FilterString(msgId);
            return ExecuteDataRow(sql);
        }
        public DataSet Get_TransactionCount(string user)
        {
            var sql = "Exec proc_TransactionCount ";
            sql += " @User =" + FilterString(user);

            return ExecuteDataset(sql);
        }

        public DataTable RegulatoryReport(string Date, string User)
        {
            var sql = "Exec proc_RegulatoryReport ";
            sql += " @DATE = " + FilterString(Date); ;
            return ExecuteDataTable(sql);
        }

        public DataTable GetTxnCountrywise()
        {
            var sql = "EXEC [proc_messageDisplay] @flag = 'getTxnCountywise'";

            return ExecuteDataTable(sql);
        }

        public DataSet GetHighChartData(string User)
        {
            var sql = "EXEC [PROC_HIGH_CHART] @flag = 'high-chart'";
            sql += " ,@User = " + FilterString(User); 
        ...
        {
            var sql = "EXEC [PROC_HIGH_CHART] @flag = 'country'";
