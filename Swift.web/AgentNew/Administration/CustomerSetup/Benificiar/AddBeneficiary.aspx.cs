...
namespace Swift.web.AgentNew.Administration.CustomerSetup.Benificiar
{
    public partial class AddBeneficiary : System.Web.UI.Page
    {
        ...
        private void PoulateLocation()
        {
            string bankId = Request.Form["pAgent"];
            string countryId = Request.Form["country"];
            string pMode = Request.Form["paymentMode"];
            ...
            DataTable dt = null;
            string sql = "EXEC proc_dropDownLists @flag = 'pickBranchById', @agentId=" + dao.FilterString(bankId);
            dt = dao.ExecuteDataset(sql).Tables[0];
            var paymentDdl = Mapper.DataTableToClass<DropDownModel>(dt);
        ...
        {
            var customerId = Request.Form["hideCustomerId"].ToString();
            if (customerId == "")
            {
        ...
        private void PopulateForm(string id)
        {
            ...
            if (null != dr)
            {
                ...
                ddlRelationship.SelectedItem.Text = dr["relationship"].ToString();
                if (ddlRelationship.SelectedItem.Text.ToUpper() == "OTHERS")
                {
                ...
                DDLBankLocation.SelectedValue = dr["bankLocation"].ToString();
        ...
        {
            DataTable result = _cd.GetCustomerDetailsWitDT(getCustomerId(), GetStatic.GetUser());

            String jsonData = GetStatic.DataTableToJson(result);
