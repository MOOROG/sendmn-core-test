...
namespace Swift.web.AgentNew.CancelRequest
{
    public partial class CancelRequest : Page
    {
        ...
        {
            string cancelResson = cancelReason.Text.ToString();
            string tranSts = lblStatus.Text.ToString();
            if (tranSts == "ModificationRequest")
            {
                GetStatic.AlertMessage(this, "Transaction in Modification State");
            }
            else
            {
                CancelRequestTran();
