...
{
    public partial class DrillDownCash : System.Web.UI.Page
    {
        ...
        private void PopulateReport()
        {
            ...
            string agentId = GetStatic.GetSettlingAgent();
        ...
        private string GetRows(DataRow[] rows, ref int sNo, ref double totalAmt)
        {
            ...
            foreach (DataRow item in rows)
            {
                ...
                sb.AppendLine("<td align='right'><a href=\"DrillDownCash.aspx?flag=drill-down&type=" + type + "&fromDate=" + GetFromDate() + "&toDate=" + GetToDate() + "&agentId=" + agentId + "&agentName=" + item["AGENT_NAME"].ToString() + "&referralName=" + item["REFERRAL_NAME"].ToString() + "\">" + GetStatic.ShowDecimal(GetDoubleValue(item["CASH_COLLECTED"].ToString()).ToString()) + "</a></ td > ");
                sb.AppendLine("</tr>");
            }
            totalAmt += totalAgent;
