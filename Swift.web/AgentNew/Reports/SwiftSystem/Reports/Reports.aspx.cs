...
namespace Swift.web.AgentNew.Reports.SwiftSystem.Reports
{
    public partial class Reports : Page
    {
        ...
        // GetStatic.ReadQueryString("toAmt", ""); var orderBy = GetStatic.ReadQueryString("orderBY",
        // ""); var isd = GetStatic.ReadQueryString("isd", ""); if (isd.ToLower().Equals("y")) {
        // totalFields = "7|8|10|11"; fieldFormat = "||||||||N||N|N||N"; fieldAlignment =
        // "||||||||R||R|R||R"; }

        // r = amlReport.CustomerReport(user, sCountry, rCountry, sAgent, rAgent, sBranch, rBranch,
        // rMode, tranType, dateType, frmDate, toDate, fromAmt, toAmt, isd, orderBy,
        // GetPageNumber().ToString(), GetStatic.GetReportPagesize(), isExportFull); }
        ...
        // mergeColumnHead = true; r = amlReport.MISReport(user, sCountry, rCountry, sAgent, rAgent,
        // sBranch, rBranch, rMode, tranType, dateType, frmDate, toDate, mrType,
        // GetPageNumber().ToString(), GetStatic.GetReportPagesize(), isExportFull); }
        ...
        // var sIdType = GetStatic.ReadQueryString("sIdType", ""); var rIdType =
        // GetStatic.ReadQueryString("rIdType", ""); var sIdNo = GetStatic.ReadQueryString("sIdNo",
        // ""); var rIdNo = GetStatic.ReadQueryString("rIdNo", ""); var rptFor =
        // GetStatic.ReadQueryString("rptFor", "");

        // var fromAmt = GetStatic.ReadQueryString("fAmt", ""); var toAmt =
        // GetStatic.ReadQueryString("tAmt", "");

        // var sCustomer = GetStatic.ReadQueryString("scust", ""); var receiverName =
        // GetStatic.ReadQueryString("recName", ""); var txnDate = GetStatic.ReadQueryString("date",
        // ""); var country = GetStatic.ReadQueryString("country", ""); var idType =
        // GetStatic.ReadQueryString("idType", ""); var idNumber =
        // GetStatic.ReadQueryString("idNumber", ""); var company =
        // GetStatic.ReadQueryString("company", ""); var senderName =
        // GetStatic.ReadQueryString("senderName", ""); var searchBy =
        // GetStatic.ReadQueryString("searchBy", ""); var membershipId =
        // GetStatic.ReadQueryString("membershipId", "");

        // totalFields = "10|12"; totalText = "Total"; fieldFormat = "||||||||||N||N||N";
        // fieldAlignment = "||||||||||R||R||R"; mergeColumnHead = true;
        ...
        // var rr = new ReportResult(); if (provider ==
        // DAL.BL.System.Utility.Utility.GetgblAgentId()) { rr = new
        // GlobalBankDao().GetReconcileReport(user, fromDate); fieldFormat = "|||||N"; fieldAlignment
        // = "|||||R"; totalFields = "5"; } else if (provider ==
        // DAL.BL.System.Utility.Utility.GetCEAgentId()) { rr = new
        // CashExpressDao().GetReconcileReport(user, fromDate, toDate); fieldFormat =
        // "||||||N|||||||||||||"; fieldAlignment = "||||||R|||||||||||||"; totalFields = "6";
        // totalText = "
        // <td>
        // </td>
        // Total"; } else if (provider == DAL.BL.System.Utility.Utility.GetxmAgentID()) { rr = new
        // XPressPayDao().GetSettlementReport(user, Convert.ToDateTime(fromDate),
        // Convert.ToDateTime(toDate)); fieldFormat = "||N|N"; fieldAlignment = "||R|R"; totalFields
        // = "2|3"; } else if (provider == DAL.BL.System.Utility.Utility.GetmgAgentId()) { rr = new
        // MoneyGramDao().GetReconcileReport(user, Convert.ToDateTime(fromDate), agentId);
        // fieldFormat = "|||||N"; fieldAlignment = "|||||R"; totalFields = "5"; } else if (provider
        // == DAL.BL.System.Utility.Utility.GetICAgentId()) { fieldFormat = "|||||N"; fieldAlignment
        // = "|||||R"; totalFields = "5";

        // rr = new InstantCashDao().GetReconcileReport(user, fromDate, toDate); } else if (provider
        // == DAL.BL.System.Utility.Utility.GetriaAgentID()) { var ds = new DataSet(); DataTable dt;
        // var res = new Ria.Pay.RiaPayDao().GetReconcileReport(GetStatic.GetUser(), fromDate, out dt);

        // var reportName = "Ria Remit Reconcile Report"; includeSerialNo = true; fieldFormat =
        // "||||||N||N|"; fieldAlignment = "||||||R||R|"; totalText = "Grand Total"; //totalFields =
        // "7|9"; totalFields = "6|8";

        // DataTable dtResult = new DataTable(); dtResult.Columns.Add("ErrorCode");
        // dtResult.Columns.Add("Msg"); dtResult.Columns.Add("Id");

        // var row = dtResult.NewRow(); row[0] = res.ErrorCode; row[1] = res.Msg; row[2] = res.Id; dtResult.Rows.Add(row);

        // DataTable dtFilter = new DataTable(); dtFilter.Columns.Add("Head"); dtFilter.Columns.Add("Value");

        // if (!string.IsNullOrWhiteSpace(fromDate)) { row = dtFilter.NewRow(); row[0] = "Date";
        // row[1] = DAL.BL.System.Utility.Utility.GetDateToShortString(fromDate);
        // dtFilter.Rows.Add(row); }

        // DataTable dtTitle = new DataTable(); dtTitle.Columns.Add("Title");

        // row = dtTitle.NewRow(); row[0] = reportName; dtTitle.Rows.Add(row);

        // ds.Tables.Add(dt); ds.Tables.Add(dtResult); ds.Tables.Add(dtFilter); ds.Tables.Add(dtTitle);

        // var dao = new SwiftDao(); rr = dao.ParseReportResult(ds); }
