...
namespace Swift.web.AgentNew.SendTxn {
  public partial class Confirm : System.Web.UI.Page {
    ...
    // string reqMethod = HttpContext.Current.Request.Form["Questionnarie"]; protected readonly
    // string _Questionnarie = GetStatic.ReadQueryString("Questionnarie", "");

    #region Get Sender Details

    protected readonly string _senderId = GetStatic.ReadQueryString("senderId", "");
    ...
    private readonly string _fxSessionId = GetStatic.ReadQueryString("fxSessionId", "");

    private readonly string _senderIsOrg = GetStatic.ReadQueryString("senderIsOrg", "");
    private readonly string _receiverIsOrg = GetStatic.ReadQueryString("receiverIsOrg", "");
    private readonly string _receiverBinn = GetStatic.ReadQueryString("receiverBinn", "");
    private readonly string _receiverBikk = GetStatic.ReadQueryString("receiverBikk", "");
    private readonly string _transactionDescription = GetStatic.ReadQueryString("transactionDescription", "");
    private readonly string _whichCur = GetStatic.ReadQueryString("whichCur", "");
    ...
    private readonly string _isRealTime = GetStatic.ReadQueryString("isRealTime", "");
    private readonly string _tpExRate = GetStatic.ReadQueryString("tpExRate", "");
    private readonly string _isExrateFromPartner = GetStatic.ReadQueryString("IsExrateFromPartner", "");
    ...
    private readonly string _iym = GetStatic.ReadQueryString("iym", "");
    private readonly string _ipp = GetStatic.ReadQueryString("ipp", "");
    ...
    private readonly string _isCDDI = GetStatic.ReadQueryString("isCDDI", "");
    private readonly string _agentRefId = GetStatic.ReadQueryString("agentRefId", "");
    private readonly string _xmlAdditionalCDDI = GetStatic.ReadQueryString("xmlAdditionalCDDI", "");

    private readonly string _controlNumber = GetStatic.ReadQueryString("controlNumber", "");
    ...
    private readonly string _calcBy = GetStatic.ReadQueryString("calcBy", "");
    private readonly string _promotionCode = GetStatic.ReadQueryString("promotionCode", "");
    private readonly string _promotionAmount = GetStatic.ReadQueryString("promotionAmount", "");

    private const string ConfirmPage = "40101460";
    ...
    private void Authenticate() {
      _sl.CheckAuthentication(ConfirmPage);
    }
    ...
    protected void ShowData() {
      ...
      pCurr1.Text = _pCurr;
      pCurr2.Text = _pCurr;
      transactionDescript.Text = _transactionDescription;
    ...
    private bool RequiredFieldValidate() {
      ...
      if (!string.IsNullOrEmpty(_senIdValid)) {
        if (Convert.ToDateTime(_senIdValid) < DateTime.Today) {
          Msg = "Sender ID has expired";
          return false;
        }
      }
    ...
    //    var agentRefId = (string.IsNullOrEmpty(_agentRefId)) ? Guid.NewGuid().ToString() : _agentRefId;
    ...
    //    trn.isAdditionalCDDI = _isCDDI;
    //    trn.CDDIXml = _xmlAdditionalCDDI;
    //    trn.controlNumber = _controlNumber;
    //    trn.calcBy = _calcBy;
    ...
    //    if (string.IsNullOrEmpty(_benId) || string.IsNullOrEmpty(_receiverId))
    //    {
    ...
    //        {
    //            var dbResult1 = _st.ParseDbResult(ds.Tables[2]);
    ...
    //            else
    //            {
    //                var dbResult2 = _st.ParseDbResult(ds.Tables[4]);
    ...
    //                if (ds.Tables[3].Rows.Count > 0 && dbResult2.ErrorCode != "0")
    //                    LoadCompliance(dbResult2, ds.Tables[3], agentRefId);
    //            }
    //            return true;
    //        }
    //        else if (dbResult.ErrorCode == "101" || dbResult.ErrorCode == "102" || dbResult.ErrorCode == "103")
    ...
    private bool LoadCompliance(DbResult dbResult, DataTable dt, string agentRefId) {
      if (dbResult.ErrorCode == "101") {
        btnProceed.Enabled = false;
        btnProceed.Visible = false;

        LoadComplianceListNew(dbResult, dt);
        return true;
      }
      ...
      if (dbResult.ErrorCode == "102") {
        ...
        //else
        //{
        //    var msgCompliance = GetStatic.ParseResultJsPrint(dbResult);
        //    GetStatic.CallBackJs1(Page, "Print Message", "ManageMessage('" + msgCompliance + "-:::-" + agentRefId + "');");
        //    return false;
        //}
      }
      if (dbResult.ErrorCode == "103") {
        btnProceed.Enabled = true;
        btnProceed.Visible = true;
        if (_isCDDI == "Y") {
          LoadComplianceListNew(dbResult, dt);
          ...
          var msgCompliance = GetStatic.ParseResultJsPrint(dbResult);
          GetStatic.CallBackJs1(Page, "Print Message", "ManageMessage('" + msgCompliance + "-:::-" + dbResult.Extra2 + "-:::-" + agentRefId + "');");
          return false;
        }
      }
      var mes = GetStatic.ParseResultJsPrint(dbResult);
    ...
    private void Proceed() {
      ...
      if (dbResult.ErrorCode == "0" || dbResult.ErrorCode == "100" || dbResult.ErrorCode == "101") {
        ...
        ManageMessageCustom(dbResult);
      } else {
        GetStatic.SetMessage(dbResult.ErrorCode, dbResult.Msg);
        ManageMessageCustom1(dbResult);
      }
    }
    ...
    private void ManageMessageCustom(JsonResponse dbResult) {
      var mes = dbResult.ErrorCode + "-:::-" + dbResult.Msg.Replace("'", "").Replace("<br/>", "").Replace(System.Environment.NewLine, "") + "-:::-" + dbResult.Id;
      mes = mes.Replace("'", "");
      mes = mes.Replace("<center>", "");
      mes = mes.Replace("</center>", "");
    ...
    private void ManageMessageCustom1(JsonResponse dbResult) {
      var mes = dbResult.ErrorCode + "-:::-" + dbResult.Msg.Replace("'", "").Replace("<br/>", "").Replace(System.Environment.NewLine, "") + "-:::-" + dbResult.Id;
      mes = mes.Replace("<center>", "");
      mes = mes.Replace("</center>", "");
    ...
    private JsonResponse Save() {
      var trn = new IRHTranDetail();
      MobileRemitRequest mrr = new MobileRemitRequest();

      mrr.User = "demo";// GetStatic.GetUser();
      mrr.SenderId = _senderId;
      mrr.ReceiverId = string.IsNullOrEmpty(_benId) ? hddReceiverId.Value : _benId.ToString();
      mrr.DeliveryMethodId = _dm == "BANK DEPOSIT" ? "2" : "0";
      mrr.PAgent = _pAgent;
      mrr.PCurr = _pCurr;
      mrr.CollCurr = _collCurr;
      mrr.CollAmt = _cAmt.ToString();
      mrr.PayoutAmt = _pAmt.ToString();
      mrr.TransferAmt = _tAmt.ToString();
      mrr.ServiceCharge = _serviceCharge.ToString();
      mrr.Discount = null;
      mrr.ExRate = _customerRate.ToString();
      mrr.CalBy = _calcBy;
      mrr.PurposeOfRemittance = _por;
      mrr.SourceOfFund = _sof;
      mrr.RelWithSender = _rel;
      mrr.Occupation = _occupation;
      mrr.IpAddress = _ipp;
      mrr.RState = _pLocation;
      mrr.RLocation = _pLocation;
      mrr.TpExRate = _tpExRate;
      mrr.TpPCurr = _pCurr;
      mrr.PayOutPartner = _payoutPartnerId;
      mrr.FOREX_SESSION_ID = _fxSessionId;
      mrr.PaymentType = "wallet";
      mrr.IsAgreed = "True";
      mrr.ProcessId = null;
      mrr.ReceiverAccountNo = _recaccountNo;
      mrr.schemeId = _schemeType;
      mrr.isUseBiometric = false;
      mrr.TxnPassword = "fromCore";
      mrr.senderIsOrg = _senderIsOrg;
      mrr.receiverIsOrg = _receiverIsOrg;
      mrr.receiverBinn = _receiverBinn;
      mrr.receiverBikk = _receiverBikk;
      mrr.transactionDesc = _transactionDescription;
      mrr.whichCur = _whichCur;
      List<result> result = new List<result>();
      result = JsonConvert.DeserializeObject<List<result>>(hddQuestinnarie.Value);
      txnCompliance txnCompliance = new txnCompliance();
      txnCompliance.result = result;
      mrr.txnCompliance = txnCompliance;
      ExchangeRateAPIService ExService = new ExchangeRateAPIService();
      JsonResponse jresp = ExService.SendTransactionContact(mrr, _pBankName);
      string dbg = JsonConvert.SerializeObject(jresp);
      return jresp;
      #region OLD MODEL
      ...
      //trn.SmsSend = _smsSend;
      //trn.ReceiverId = string.IsNullOrEmpty(_benId) ? hddReceiverId.Value : _benId.ToString();
      //trn.RecFirstName = _recfName;
      //trn.RecMiddleName = _recmName;
      //trn.RecLastName = _reclName;
      //trn.RecLastName2 = _reclName2;
      ...
      //trn.isAdditionalCDDI = _isCDDI;
      //trn.CDDIXml = hddXMLCDDI.Value.Replace("%3e", "<").Replace("%3c", ">");
      //trn.controlNumber = _controlNumber;
      //trn.calcBy = _calcBy;

      //trn.promotionCode = _promotionCode;
      //trn.promotionAmount = _promotionAmount;

      //trn.ComplaincrDataStr = hddQuestinnarie.Value.Replace("%3e", "<").Replace("%3c", ">");
      //if (_isRealTime == "true") {
      //  DbResult _dbRes = new DbResult();
      //  string dbgs = JsonConvert.SerializeObject(trn);
      //  if (_isExrateFromPartner == "true") {
      //    _dbRes = _st.CreateTransactionFromMobileOROnline(trn, _fxSessionId);
      //    _dbRes = _st.SendTransactionIRHNew(trn);
      //  } else {
      //    _dbRes = _st.SendTransactionIRH(trn);
      //  }
      ...
      //    var result = at.GetHoldedTxnForApprovedByAdmin(GetStatic.GetUser(), _dbRes.Extra, newSession);

      //    if (!result.ResponseCode.Equals("0")) {
      //      _st.RevertTPTxns(_dbRes.Id, GetStatic.GetUser());
      //      return new DbResult {
      ...
      //      };
      //    } else {
      //      trn.tpRefNo = result.Extra;

      //      if (result.Msg.ToString().Contains("|")) {
      //        trn.tpTranId = Convert.ToString(result.Msg.Split('|')[1]);
      //      }

      //      return _st.UpdateTPTxns(trn, _dbRes.Id, GetStatic.GetUser());
      //    }
      //  } else {
      //    return _dbRes;
      //  }
