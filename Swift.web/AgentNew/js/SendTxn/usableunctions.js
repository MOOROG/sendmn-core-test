...
function ParseCalculateDataMine(response, amtType) {
  var data = response;
  $("#" + mId + "lblSendAmt").val(parseFloat(Number(data.sAmt).toFixed(3))); //
  $("#" + mId + "lblExRate").text(roundNumber(data.exRate, 11));
  $("#" + mId + "lblPayCurr").text(data.pCurr);
  $("#" + mId + "lblExCurr").text(data.pCurr);

  if ($("#" + mId + "allowEditSC").val() === 'Y') {
    $("#" + mId + "editServiceCharge").attr("disabled", false);
  }

  $("#" + mId + "lblPerTxnLimit").text(data.maxAmountLimitPerTran);

  if (!$("#" + mId + "editServiceCharge").is(':checked')) {
    $("#" + mId + "lblServiceChargeAmt").attr('disabled', 'disabled');
  }

  $("#" + mId + "lblServiceChargeAmt").val(parseFloat(data.scCharge).toFixed(0));


  SetValueById(mId + "txtCollAmt", parseFloat(Number(data.collAmt).toFixed(3)), ""); //

  SetValueById(mId + "lblSendAmt", parseFloat(Number(data.sAmt).toFixed(3)), ""); //
  SetValueById(mId + "txtPayAmt", parseFloat(Number(data.pAmt).toFixed(2)), "");

  $("#" + mId + "hddTPExRate").val(data.tpExRate);
  $("#" + mId + "forexSessionId").val(data.EXRATEID);
}
...
function CheckSession(data) {
  if (data === undefined || data === "" || data === null || Object.keys(data).length == 0)
        return;
