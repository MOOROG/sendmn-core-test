...
namespace Swift.web.AgentPanel.International.SendMoneyv2
{
    public partial class SendV2 : Page
    {
        ...
        protected void Calculate()
        {
            ...
            // ExchangeRate exRate = new ExchangeRate(); ExchangeRateAPIService ExService = new
            // ExchangeRateAPIService(); exRate.RequestedBy = "core"; exRate.SCountry =
            // Convert.ToInt32(GetStatic.GetCountryId()); exRate.SSuperAgent =
            // Convert.ToInt32(GetStatic.GetSuperAgent()); exRate.SAgent =
            // Convert.ToInt32(GetStatic.GetAgent()); exRate.SBranch =
            // Convert.ToInt32(GetStatic.GetBranch()); exRate.PCountry =
            // Convert.ToInt32(Request.Form["pCountry"]); //exRate.PCountryName =
            // Request.Form["pCountrytxt"]; exRate.ServiceType = Request.Form["pMode"];
            // exRate.PaymentType = Request.Form["pModetxt"]; exRate.PAgent =
            // Convert.ToInt32(Request.Form["pAgent"]); exRate.CAmount =
            // Convert.ToDecimal(Request.Form["collAmt"]); exRate.PAmount =
            // Convert.ToDecimal(Request.Form["payAmt"]); exRate.CollCurrency =
            // Request.Form["collCurr"]; exRate.PCurrency = Request.Form["payCurr"];
            // exRate.CustomerId = Convert.ToInt32(Request.Form["senderId"]); exRate.SchemeId =
            // Request.Form["schemeCode"] == "" ? 0 : Convert.ToInt16(Request.Form["schemeCode"]);
            // exRate.ForexSessionId = Request.Form["couponId"]; exRate.IsManualSc =
            // (Request.Form["isManualSc"] == "N" ? false : true); exRate.ManualSc = Convert.ToDecimal(Request.Form["sc"]);
