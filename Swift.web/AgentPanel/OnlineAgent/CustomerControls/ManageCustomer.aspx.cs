...
namespace Swift.web.AgentPanel.OnlineAgent.CustomerControls
{
    public partial class ManageCustomer : System.Web.UI.Page
    {
        ...
        {
            var dr = _cd.GetCustomerDetailsForEdit(eId, GetStatic.GetUser());
            if (null != dr)
            {
                ...
                {
                    msgDiv.Visible = true;
                    msgLabel.Text = "Note: The customer has already made transactions in JME system, so please refer with HO to modify transactions.";
                }
        ...
        protected void register_Click(object sender, EventArgs e)
        {
            if (email.Text.Equals(emailConfirm.Text))
            {
                ...
                {
                    GetStatic.AlertMessage(this, "File size exceeded for Alien Reg ID Front. Please upload image of size less than 2mb.");
                    return;
                }
                string verDoc2 = UploadDocument(VerificationDoc2, hddIdNumber.Value, 2000);
                if (verDoc2 == "invalidSize")
                {
                    GetStatic.AlertMessage(this, "File size exceeded for Alien Reg ID Back. Please upload image of size less than 2mb.");
                    return;
                }
                ...
                var dbResult = _cd.ModifyCustomer(customerModel);
