...
{
    public partial class drilDown : System.Web.UI.Page
    {
        ...
        private void LoadSoaHtml()
        {
            ...
            lblAgentName.Text = rl.GetAgentNameByMapCodeInt(agent);
            lblFrmDate.Text = fromDate;
            lbltoDate.Text = toDate;
            lblGeneratedDate.Text = DateTime.Now.ToString("MM/dd/yyyy");

            DataTable dt = rptDao.AgentSoaDrilldownReport(fromDate, toDate, agent, flag, TRAN_TYPE);

            int cols = dt.Columns.Count;
            var str = new StringBuilder("<table class='TBLReport table table-bordered table-condensed' border=\"1\" cellspacing=0 cellpadding=\"3\">");
