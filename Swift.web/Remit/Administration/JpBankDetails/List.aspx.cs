...
namespace Swift.web.Remit.Administration.JpBankDetails
{
    public partial class List : System.Web.UI.Page
    {
        ...
        private const string MapFunctionId = "20306010";
        private const string UnMapFunctionId = "20306020";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (MethodName == "PopulateHoldTxnList")
            {
                PopulateHoldTxnList();
            }
            else if (MethodName == "SaveMapping")
            {
                SaveMapping();
            }
            if (!IsPostBack)
        ...
        private void SaveMapping()
        {
            ...
            DbResult _dbRes = _dao.SaveMapping(GetStatic.GetUser(), tranId, id);
            GetStatic.JsonResponse(_dbRes, Page);
        }

        private void PopulateHoldTxnList()
        {
            DataTable dt = _dao.GetHoldTxnList(GetStatic.GetUser());
            GetStatic.DataTableToJson(dt, this);
        }
        ...
        private void PopulateData()
        {
            try
            {
                ...
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    sb.AppendLine("<td>" + GetLinkText(item["tranId"].ToString(), item["processedBy"].ToString(), item["approvedBy"].ToString(), hasMapRole, hasUnMapRole, GetStatic.ShowDecimal(item["depositAmount"].ToString()), item["particulars"].ToString()) + "</td>");
                    sb.AppendLine("</tr>");
        ...
        private string GetLinkText(string rowId, string processedBy, string approvedBy, bool hasMapRole, bool hasUnMapRole, string amt, string particulars)
        {
            ...
            if (approvedBy.ToString() == "")
            {
               ...
               + "</a>";
                linkText += "&nbsp;<a title=\"Edit\" href=\"javascript:void(0);\" " + map + ">"
                   + "<span class=\"action-icon\">"
                       + "<btn class=\"btn btn-xs btn-success\" onclick=\"return MapTxn('" + rowId + "', '" + amt + "', '" + particulars + "')\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Map Existing Txn\">"
