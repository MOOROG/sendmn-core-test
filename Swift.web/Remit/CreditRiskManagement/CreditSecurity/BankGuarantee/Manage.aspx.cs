...
namespace Swift.web.Remit.CreditRiskManagement.CreditSecurity.BankGuarantee
{
    public partial class Manage : Page
    {
        ...
        private readonly BankGuaranteeDao obj = new BankGuaranteeDao();
        ...
        private long GetId()
        {
            return GetStatic.ReadNumericDataFromQueryString("bgId");
        }
        ...
        private void PopulateDataById()
        {
            ...
            guaranteeNo.Text = dr["guaranteeNo"].ToString();
            amount.Text = dr["amount"].ToString();
            bankName.Text = dr["bankName"].ToString();
            issuedDate.Text = dr["issuedDate"].ToString();
        ...
        private void Update()
        {
            DbResult dbResult = obj.Update(GetStatic.GetUser(), GetId().ToString(), GetAgentId().ToString(),
                                           guaranteeNo.Text, amount.Text, currency.Text, bankName.Text, issuedDate.Text,
                                           expiryDate.Text, followUpDate.Text, GetStatic.GetSessionId());
        ...
        private void Upload()
        {
            ...
            if (fileUpload.PostedFile.FileName != null)
            {
                ...
                info = UploadFile(guaranteeNo.Text + "." + type, GetId().ToString(), root);

                if (info.Substring(0, 5) == "error")
                    return;

                DbResult dbResult = csd.Update(GetStatic.GetUser(), GetSdId().ToString(), GetId().ToString(), "B",
