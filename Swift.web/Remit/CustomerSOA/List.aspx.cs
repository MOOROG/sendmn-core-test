...
namespace Swift.web.Remit.CustomerSOA
{
    public partial class List : System.Web.UI.Page
    {
        ...
        protected void btnPrintReceipt_Click(object sender, EventArgs e)
        {
            ...
            var fromDate = DateTime.Parse(Request.Form["fromDate"]).ToString("yyyy-MM-dd");
            var toDate = DateTime.Parse(Request.Form["toDate"]).ToString("yyyy-MM-dd");
            var custId = GetCustomerId("CustomerID"); //should be same present in  grid filter key

            GetStatic.WriteSession("tranIds", trnids);
            GetStatic.WriteSession("custId", custId);

            string url = "CustomerSoaReceipt.aspx?fromDate=" + fromDate + "&toDate=" + toDate;
            Response.Write("<script type='text/javascript'>window.open('" + url + "');</script>");
            //Response.Redirect("CustomerSoaReceipt.aspx?fromDate=" + fromDate + "&toDate=" + toDate);
        }

        private string GetCustomerId(String key)
        {
            var usr = GetStatic.GetUser();
            var strClientID = GridName + "_" + key;
