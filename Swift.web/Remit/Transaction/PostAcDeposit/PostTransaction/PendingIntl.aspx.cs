...
namespace Swift.web.Remit.Transaction.PayAcDepositV3.PostTransaction
{
    public partial class PendingIntl : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            var ds = _obj.GetPendingUnpaidIntl(GetStatic.GetUser(), hdnPAgent.Value, fromDate, toDate, fromTime, toTime);
            if (ds == null)
                return;
            var dt = ds.Tables[0];
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    str.Append("<td align=\"center\"><input type='checkbox' id = \"chk_" + dr["Tran No"] + "\" name = \"chkId\" value='" + dr["Tran No"] + "' /></td>");
                    for (int i = 0; i < cols; i++)
                    {
                        if (i == 7)
                        {
                            str.Append("<td align=\"left\"><a href=\"PostIntl.aspx?pAgentName=" + hdnPAgentName.Value + "&tranId=" + dr["Tran No"] + "&pAgent=" + hdnPAgent.Value
                    ...
                    str.Append("</tr>");
                    totalRec++;
                    totalAmt += double.Parse(dr[9].ToString());
                }
        ...
        {
            var dbResult = _obj.PostIntl(GetStatic.GetUser(), _tranNo, hdnPAgent.Value);
