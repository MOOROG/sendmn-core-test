...
namespace Swift.web.Remit.Transaction.Send.Domestic
{
    public partial class FormLoader : System.Web.UI.Page
    {
        ...
        private const string AddEditFunctionId = "20121310";
        ...
        private void PopulateBranchName()
        {
            ...
            if (dt == null || dt.Rows.Count == 0)
            {
                Response.Write("<select id=\"bankBranch\" class='form-control' style=\"Width:240px;\" \"></select>");
                return;
            }
            var html = new StringBuilder("<select id=\"bankBranch\" class='form-control' style=\"Width:240px;\" >");
            html.Append("<option value = \"\">Select</option>");
            foreach (DataRow dr in dt.Rows)
            {
        ...
        {
            var dbResult = send.HoVerifyDomesticTransaction(GetStatic.GetUser(), _amount, _sBranch);
            if(dbResult.ErrorCode != "0")
            {
                Response.Write(dbResult.ErrorCode + "|" + dbResult.Msg + "|" + dbResult.Id);
                return;
