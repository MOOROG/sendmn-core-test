function ViewStatementReport(type) {
    ...
    if (type == 're') {
        ...
        reqField = "startDate,endDate,acInfo_aText,";
        if (ValidRequiredField(reqField) === false) {
            return false;
        }
        $('#accDetails').text($('#acInfo_aText').val());
    ...
    var dataObject = {
        ...
        FromDate: startDate,
        ToDate: endDate,
        accNum: $('#acInfo_aValue').val(),
        accName: $('#acInfo_aText').val(),
    ...
    $.post(url, dataObject, function (data) {
        ...
        $.each(result, function (i, d) {
            ...
            //if (d['hasRight'] && sn > 0) {
            //    url = 'userreportResultSingle.aspx?r=Y&company_id=1&vouchertype=' + d["tran_type"] + '&type=trannumber&trn_date=' + d["tran_date"] + '&tran_num=' + d["ref_num"];
            //    drLink = '<a class="btn btn-danger" href="javascript:void(0)" onclick="OpenInNewWindow(\'' + url + '\')"';
            //    drLink += ' title="Transaction info">Reverse</a>';
